...
**1. 安装** —— Android 7.0+(minSdk 24),把 APK 拖进手机安装。首次进入是空的"最近"列表。

**2. 打开 / 导入文档** —— 首页点 **打开 / 导入** → 选手机里的 `.md` / `.html` / `.zip`。文件会复制一份进 App 本地库(原文件不动),之后在"最近"里随时再开。

**3. 阅读** ——
- 点右上 **Aa**:切阅读主题(纸白 / 米黄 / 夜黑…)、字体、字号、代码配色。
- 点 **目录(TOC)**:按标题快速跳转;阅读进度自动记住,下次接着读。
- 正文里的图片:点一下全屏,双指捏合放大。

**4. 编辑(Markdown)** —— 阅读 `.md` 时点右上 **✏️** 进编辑,直接改源码;退出自动保存回本地库,可再 **分享 / 导出**。

**5. 整理:标签 & 收藏** ——
- 长按文件卡 → 加 **标签**、设 **收藏**、重命名、删除。
- 主页标签 chip 条按标签筛选;🗂 管理(改名 / 删标签)。想清爽可在 设置 ⚙ 关掉"卡片上显示标签"。

**6. HTML / 网页包(ZIP)** —— `.html` 直接渲染(JavaScript 默认关,需要时一键开);`.zip` 网页包解压后正确加载内部相对资源(图片 / CSS)。

> 🔒 全程纯本地:不联网、不上传、不要账号。文件只在你手机里。
